...
export default function SaleClient() {
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("richhouse-technic-lang");
  ...
  useEffect(() => {
    window.localStorage.setItem("richhouse-technic-lang", lang);
  }, [lang]);
    ...
    async function load() {
      try {
        ...
        const [settingRes, productsRes] = await Promise.all([
          fetch(`${STRAPI_URL}/api/tech-setting?populate=*`, {
            cache: "no-store",
          }),
          fetch(
            `${STRAPI_URL}/api/tech-products?populate=*&pagination[pageSize]=200&sort=sortOrder:asc`,
            {
              cache: "no-store",
            },
          ),
        ...
        console.error("Technic page load error:", error);
